...
local function saveToFile()
		...
		for i = 1, table.getn(areaTable), 1 do
				local name = '"' .. areaTable[i][1] .. '"'
				local x = tostring(areaTable[i][2])
				local y = tostring(areaTable[i][3])
				local shape = '{' .. tostring(areaTable[i][4][1]) .. ', ' .. tostring(areaTable[i][4][2])
				if table.getn(areaTable[i][4]) > 2 then
						shape = shape .. ', ' .. tostring(areaTable[i][4][3])
				end
				shape = shape .. '}'
				local tier = ''
				...
				if bit.band(areaTable[i][5], 0xF0) == 0x10 then
						tier = tier .. ' + WORLDSPAWNAREA'
				elseif bit.band(areaTable[i][5], 0xF0) == 0x20 then
						tier = tier .. ' + NOWORLDSPAWNAREA'
				else
						tier = tier .. ' + ALLOWWORLDSPAWNS'
				end
				if bit.band(areaTable[i][5], 0xF00) == 0x100 then
						tier = tier .. ' + NOBUILDZONEAREA'
				else
						tier = tier .. ' + ALLOWBUILDINGS'
				end
				local spawnGroups = '{'
				for j = 1, table.getn(areaTable[i][6] or {}), 1 do
						spawnGroups = spawnGroups .. '"' .. areaTable[i][6][j] .. '"'
						if j < table.getn(areaTable[i][6] or {}) then
								spawnGroups = spawnGroups .. ', '
						end
				end
				spawnGroups = spawnGroups .. '}'
				f:write('\t{' .. name .. ', ' .. x .. ', ' .. y .. ', ' .. shape .. ', ' .. tier .. ', ' .. spawnGroups .. '}')
				if i < table.getn(areaTable) then
						f:write(',\n')
				else
						f:write('\n')
				end
