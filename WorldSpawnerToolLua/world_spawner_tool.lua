...
local function changedAreaEventHandler()
		...
		areaTable[selectedAreaIndex][7] = selectedArea["numberofspawns"]
    WorldSpawnerWindow:setAreaShapes(getAreaShapes())
...
local function saveToFile()
		...
		f:write('-- This file has been generated with the SWGEmu World Spawner Tool.\n')
		f:write('--\n')
		f:write('-- {"regionName", xCenter, yCenter, shape and size, tier, {"spawnGroup1", ...}, maxSpawnLimit}\n')
		f:write('-- Shape and size is a table with the following format depending on the shape of the area:\n')
		f:write('--   - Circle: {1, radius}\n')
		f:write('--   - Rectangle: {2, width, height}\n')
		f:write('--   - Ring: {3, inner radius, outer radius}\n')
		f:write('-- Tier is a bit mask with the following possible values where each hexadecimal position is one possible configuration.\n')
		f:write('-- That means that it is not possible to have both a spawn area and a no spawn area in the same region, but\n')
		f:write('-- a spawn area that is also a no build zone is possible.\n')
		f:write('\n')
		...
		for i = 1, table.getn(areaTable), 1 do
				...
				if areaTable[i][7] and areaTable[i][7] ~= '-' then
						table.insert(row, tostring(areaTable[i][7]))
				end
